...
| 15 migration Supabase `0001`→`0015` | ⬜ `0014` và `0015` CHƯA chạy — xem bên dưới |
...
| `0015_fix_profiles_role_values.sql` | **Sửa lỗi của `0011`.** `alter table … add column if not exists role` bị Postgres bỏ qua vì cột `role` đã có sẵn (giá trị `'user'`), nên cả `default 'officer'` lẫn ràng buộc `check` đều không được tạo — khiến `private.is_team_leader()` không bao giờ khớp và cơ chế trưởng nhóm coi như không tồn tại. Migration chuẩn hóa `'user'` → `'officer'`, đặt lại DEFAULT/NOT NULL và tạo `check` bằng `add constraint`. Chỉ cần chạy nếu định dùng vai trưởng nhóm |
