...
| `lgdesign` | 53/53 đạt | Màn đăng nhập sau khi thiết kế lại, đo trên 4 khổ màn (1440/1024/390/360): máy tính hai nửa nằm cạnh nhau, điện thoại xếp dọc; ô nhập email và nút Đăng nhập luôn nằm TRONG màn hình (không phải cuộn mới bấm được); nút cao ≥44px; còn đủ id cho JS (`login-email/pass/commit/btn/err`), có `<label for>` cho hai ô nhập, còn link sổ tay và câu mô tả tool; mọi chữ ≥4.5:1 kể cả trên nền gradient; **chữ trên nút lúc bị vô hiệu hoá vẫn đọc được** (bản đầu 1.42:1 — mất hẳn chữ) |
